...
import { getOrgMembers, getOrganization, updateOrgMemberRole, removeOrgMember } from '@/actions/organizations'
import { toast } from 'sonner'
import { MoreHorizontal, Crown, ShieldCheck, Shield, User, Trash2 } from 'lucide-react'

type OrgRole = 'owner' | 'admin' | 'moderator' | 'member'

const ROLE_BADGE: Record<OrgRole, { label: string; variant: 'default' | 'secondary' | 'outline' }> = {
...
export default function OrganizationMembersPage({ params }: { params: { id: string } }) {
    ...
    function handleRoleChange(profileId: string, newRole: 'admin' | 'moderator' | 'member') {
        startTransition(async () => {
            const result = await updateOrgMemberRole(params.id, profileId, newRole)
            if (result?.error) {
    ...
    function handleRemove(profileId: string) {
        startTransition(async () => {
            const result = await removeOrgMember(params.id, profileId)
            if (result?.error) {
                toast.error(result.error)
            } else {
                ...
                <CardHeader>
                    <div className="flex items-center justify-between">
                        ...
                        <Button variant="outline" asChild>
                            <Link href={`/orgs/${params.id}`}>Back to Org</Link>
                        </Button>
                    </div>
                </CardHeader>
                <CardContent className="space-y-4">
                    ...
                    <div className="space-y-2">
                        {filteredMembers.map((member) => {
                            ...
                            const canModify = canManage && member.role !== 'owner'
                            const canPromoteAdmin = myRole === 'owner'
                                        ...
                                        {canModify && (
                                            <DropdownMenu>
                                                ...
                                                <DropdownMenuContent align="end">
                                                    {canPromoteAdmin && member.role !== 'admin' && (
